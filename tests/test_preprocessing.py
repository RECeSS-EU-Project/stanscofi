...
class TestPreprocessing(unittest.TestCase):
    ...
    def test_preprocessing_routine(self):
        dataset = self.generate_dataset(ntype_feature=2)
        X_true, y_true, scalerS, scalerP = stanscofi.preprocessing.meanimputation_standardize(dataset, subset=None, scalerS=None, scalerP=None, inf=int(1e1), verbose=False)
        X, y, _, _, filter_ = stanscofi.preprocessing.preprocessing_routine(dataset, "meanimputation_standardize", subset_=None, filter_=None, scalerS=None, scalerP=None, inf=int(1e1), njobs=1)
        self.assertEqual(X.shape[0],X_true.shape[0])
        self.assertEqual(X.shape[1],X_true.shape[1])
        self.assertTrue((X==X_true).all())
        self.assertEqual(y.shape[0],y_true.shape[0])
        ...
        X_true, y_true, scalerS, scalerP = stanscofi.preprocessing.meanimputation_standardize(dataset, subset=2, scalerS=None, scalerP=None, inf=int(1e1), verbose=False)
        X, y, _, _, _ = stanscofi.preprocessing.preprocessing_routine(dataset, "meanimputation_standardize", subset_=2, filter_=None, scalerS=scalerS, scalerP=scalerP, inf=int(1e1), njobs=1)
        self.assertEqual(X.shape[0],X_true.shape[0])
        self.assertEqual(X.shape[1],X_true.shape[1])
        ...
        self.assertTrue((y==y_true).all())
        dataset = self.generate_dataset(same_item_user_features=True,ntype_feature=2)
        X_true, y_true = stanscofi.preprocessing.same_feature_preprocessing(dataset)
        X, y, _, _, _ = stanscofi.preprocessing.preprocessing_routine(dataset, "same_feature_preprocessing", subset_=None, filter_=None, scalerS=None, scalerP=None, inf=int(1e1), njobs=1)
        self.assertEqual(X.shape[0],X_true.shape[0])
        self.assertEqual(X.shape[1],X_true.shape[1])
        self.assertTrue((X==X_true).all())
        self.assertEqual(y.shape[0],y_true.shape[0])
        ...
        self.assertEqual(len(y.shape), 1)
        self.assertTrue((y==y_true).all())
        X, y, _, _, filter_ = stanscofi.preprocessing.preprocessing_routine(dataset, "same_feature_preprocessing", subset_=2, filter_=None, scalerS=None, scalerP=None, inf=int(1e1), njobs=1)
        self.assertEqual(X.shape[1], 2)
        self.assertTrue(len(filter_)==2)
        ntype_feature = 2
        dataset = self.generate_dataset(sep_feature="--", ntype_feature=ntype_feature)
        X_true, y_true = stanscofi.preprocessing.Perlman_procedure(dataset, njobs=5, sep_feature="--")
        X, y, _, _, _ = stanscofi.preprocessing.preprocessing_routine(dataset, "Perlman_procedure", subset_=None, filter_=None, scalerS=None, scalerP=None, inf=int(1e1), njobs=5)
        self.assertEqual(X.shape[0],X_true.shape[0])
        self.assertEqual(X.shape[1],X_true.shape[1])
        self.assertTrue((X==X_true).all())
        self.assertEqual(y.shape[0],y_true.shape[0])
